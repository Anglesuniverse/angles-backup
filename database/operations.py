...
class DatabaseOperations:
    ...
    async def store_conversation(
        ...
        try:
            ...
            result = await self.client.insert_data("conversations",
                                                   validated_data)
    ...
    async def get_relevant_memories(self,
        ...
        try:
            ...
            memories = await self.client.query_data("memories",
                                                    limit=limit * 2)
            ...
            for memory in memories:
                ...
                tags = json.loads(memory.get(
                    "tags", "[]")) if memory.get("tags") else []

                # Check if any keyword appears in content or tags
                for keyword in keywords:
                    if keyword.lower() in content or keyword.lower() in [
    ...
    async def get_data_statistics(self) -> Dict[str, Any]:
        ...
        try:
            for table_name in self.config.get_all_table_names():
                try:
                    data = await self.client.query_data(table_name, limit=1000
                                                        )  # Sample for stats
                    stats[table_name] = {
                        "count":
                        len(data),
                        "latest_timestamp":
                        max([item.get("timestamp", "")
                             for item in data]) if data else None
                    }
                except Exception as e:
                    self.logger.warning(
                        f"Failed to get stats for table {table_name}: {str(e)}"
                    )
...
async def store_architect_decision(
    self,
    decision_type: str,
    ...
    try:
        data = {
            ...
            "source_of_decision": source_of_decision,
            "decision_category": decision_category,
            "decision_scope": decision_scope,
            "impact_level": impact_level,
            "related_agent_or_module": related_agent_or_module,
            "created_at": datetime.utcnow().isoformat()
        }

        validated_data = validate_data(data, self.config)
        result = await self.client.insert_data("architects_table",
                                               validated_data)
        self.logger.info("Stored architect decision")
        return result

    except Exception as e:
        self.logger.error(f"Failed to store architect decision: {str(e)}")
        raisel
